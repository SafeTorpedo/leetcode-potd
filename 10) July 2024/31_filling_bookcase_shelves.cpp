...
public:
    int minHeightShelves(vector<vector<int>> &books, int shelfWidth)
    {
        ...
        return solve(books, shelfWidth, memo, 0, shelfWidth, 0);
    }

private:
    int solve(vector<vector<int>> &books, int shelfWidth, vector<vector<int>> &memo, int i, int remainingShelfWidth, int maxHeight)
    {
        ...
        {
            int option1Height = maxHeight + solve(books, shelfWidth, memo, i + 1, shelfWidth - currentBook[0], currentBook[1]);
            if (remainingShelfWidth >= currentBook[0])
            {
                int option2Height = solve(books, shelfWidth, memo, i + 1, remainingShelfWidth - currentBook[0], maxHeightUpdated);
